...
function FormField({ label, value, onChange, placeholder, isMesAno }: { label: string; value: string; onChange: (v: string) => void; placeholder?: string; isMesAno?: boolean }) {
  return (
    <div className="flex flex-col gap-0.5">
      <label className="text-[10px] text-gray-800 font-medium">{label}</label>
      <input
        type="text"
        value={value}
    ...
    <div className="min-h-screen bg-gray-950 text-gray-100">
      ...
      <div className="bg-gray-900 border-b border-gray-800 px-3 py-2">
        <div className="flex flex-wrap gap-2">
          <div>
            <label className="text-[10px] text-gray-800 block mb-0.5">Mês/Ano</label>
          ...
          <div>
            <label className="text-[10px] text-gray-800 block mb-0.5">Empresa</label>
          ...
          <div>
            <label className="text-[10px] text-gray-800 block mb-0.5">Tipo Pagto</label>
          ...
          <div>
            <label className="text-[10px] text-gray-800 block mb-0.5">Nome</label>
      ...
      {totalPages > 1 && (
        <div className="flex items-center justify-center gap-2 py-2 bg-gray-900 border-b border-gray-800">
          <Button size="sm" variant="outline" disabled={page === 1} onClick={() => setPage(p => p - 1)} className="h-6 px-2 text-[10px] bg-gray-800 border-gray-700 text-gray-800">Anterior</Button>
          <span className="text-[10px] text-gray-800">Pág. {page} / {totalPages}</span>
          <Button size="sm" variant="outline" disabled={page >= totalPages} onClick={() => setPage(p => p + 1)} className="h-6 px-2 text-[10px] bg-gray-800 border-gray-700 text-gray-800">Próxima</Button>
        </div>
      )}
      {/* Tabela */}
      <div className="overflow-x-auto">
        <table className="w-full text-[11px] border-collapse">
          ...
          <tbody>
            {rows.length === 0 ? (
              <tr><td colSpan={5} className="text-center py-10 text-gray-800">Nenhum registro encontrado.</td></tr>
            ) : rows.map((row, i) => {
              ...
              return (
                <tr key={row.id} className={`border-b border-gray-800/60 transition-colors ${sel ? "bg-purple-900/25 hover:bg-purple-900/35" : isAtrasado(row.dataVencer) && !row.dataPagto ? "bg-red-950/60 hover:bg-red-900/50 border-red-900/40" : i % 2 === 0 ? "bg-gray-900/50 hover:bg-gray-800/40" : "bg-gray-900/20 hover:bg-gray-800/40"}`}>
                  ...
                  <td className="px-2 py-1.5 text-right whitespace-nowrap">
                    ...
                    <div className="text-[10px] text-gray-800">Vence: {row.dataVencer || "-"}</div>
                  </td>
          ...
          {rows.length > 0 && (
            <tfoot>
              <tr className="bg-gray-800 font-semibold border-t-2 border-purple-600">
                <td colSpan={2} className="px-2 py-1.5 text-right text-[10px] text-gray-800">Total:</td>
        ...
        <div className="fixed inset-0 bg-black/70 flex items-center justify-center z-50 p-4">
          <div className="bg-gray-900 border border-gray-700 rounded-lg w-full max-w-2xl max-h-[90vh] overflow-y-auto shadow-2xl">
            <div className="flex items-center justify-between px-4 py-3 border-b border-gray-700">
              ...
              <button onClick={() => setModal(m => ({ ...m, open: false }))} className="text-gray-800 hover:text-white text-lg leading-none">×</button>
            </div>
            <div className="p-4 grid grid-cols-2 gap-3">
              <FormField label="Mês Ano" value={modal.dados.mesAno ?? ""} onChange={v => setField("mesAno", v)} placeholder="MM/AAAA" isMesAno />
              <div className="flex flex-col gap-0.5">
                <label className="text-[10px] text-gray-800 font-medium">Tipo Pagto</label>
                <select
              ...
              <div className="flex flex-col gap-0.5">
                <label className="text-[10px] text-gray-800 font-medium">Cidade Resp.</label>
              ...
              <div className="col-span-1">
                <label className="text-[10px] text-gray-800 font-medium">Banco</label>
                <BancoSelect
              ...
              {modal.modo === "novo" && (
                <div className="flex flex-col gap-0.5 col-span-2">
                  <label className="text-[10px] text-gray-800 font-medium">Repetir por quantos meses?</label>
            ...
            <div className="flex justify-end gap-2 px-4 py-3 border-t border-gray-700">
              <Button onClick={() => setModal(m => ({ ...m, open: false }))} size="sm" variant="outline" className="h-7 px-3 text-xs bg-gray-800 border-gray-600 text-gray-800">Cancelar</Button>
