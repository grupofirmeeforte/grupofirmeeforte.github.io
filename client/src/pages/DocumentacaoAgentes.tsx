...
function getFileIcon(tipo: string) {
  ...
  return <File className="w-5 h-5 text-gray-900" />;
}
    ...
    return (
      <div className="min-h-screen bg-gray-950 text-white">
            ...
            <div>
              <div className="flex items-center gap-2">
                ...
                {agenteSelecionado.empresa && (
                  <span className="text-xs text-gray-900 bg-gray-100 px-2 py-0.5 rounded">{agenteSelecionado.empresa}</span>
                )}
              </div>
              <p className="text-sm text-gray-900">Documentos do agente</p>
            </div>
          </div>
        ...
        <div className="px-6 py-3 bg-gray-900 border-b border-gray-700 flex gap-3 items-center">
          ...
          <span className="text-sm text-gray-900">{docsFiltrados.length} documento(s)</span>
        </div>
          ...
          <div className="bg-gray-900 rounded-lg shadow overflow-hidden border border-gray-700">
            <div className="overflow-x-auto">
              <table className="w-full text-sm">
                ...
                <tbody>
                  {loadingDocs ? (
                    <tr><td colSpan={6} className="text-center py-8 text-gray-900">Carregando...</td></tr>
                    ...
                    docsFiltrados.map((doc, i) => (
                      <tr key={doc.id} className={i % 2 === 0 ? 'bg-gray-900' : 'bg-gray-800'}>
                        <td className="px-4 py-3">
                          <span className={`px-2 py-1 rounded-full text-xs font-semibold ${CORES_TIPO[doc.tipoDocumento] ?? 'bg-gray-100 text-gray-900'}`}>
                            {doc.tipoDocumento}
                          </span>
                        </td>
                        <td className="px-4 py-3">
                        ...
                        <td className="px-4 py-3 text-gray-900">{doc.tamanho ? formatBytes(doc.tamanho) : '—'}</td>
            ...
            <div className="space-y-4 py-2">
              ...
              <div>
                <label className="block text-sm font-medium text-gray-900 mb-1">
                  ...
                  <span className="text-gray-900 font-normal ml-1">(PDF, JPG, PNG — máx. 10MB)</span>
                </label>
                <div
                ...
                >
                  {arquivoSelecionado ? (
                    <div className="flex items-center justify-center gap-2 text-green-700">
                      ...
                      <span className="text-sm text-gray-900">({formatBytes(arquivoSelecionado.size)})</span>
                    </div>
                  ) : (
                    <div className="text-gray-900">
                      <Upload className="w-8 h-8 mx-auto mb-2 text-gray-900" />
          ...
          return (
            <div className="space-y-3">
              <div className="flex flex-wrap gap-2 text-sm">
                <span className={`px-2 py-1 rounded text-xs font-semibold ${CORES_TIPO[docVisualizar.tipoDocumento] ?? 'bg-gray-100 text-gray-900'}`}>
                  {docVisualizar.tipoDocumento}
                </span>
                {docVisualizar.descricao && <span className="text-gray-900">{docVisualizar.descricao}</span>}
              </div>
  ...
  return (
    <div className="min-h-screen bg-gray-950 text-white">
      ...
      <div className="px-6 py-3 bg-gray-900 border-b border-gray-700 flex gap-3 items-center">
        ...
        <span className="text-sm text-gray-900">{agentesComDocs.length} agente(s)</span>
        <span className="text-xs text-gray-900">
        ...
        <div className="bg-gray-900 rounded-lg shadow overflow-hidden border border-gray-700">
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              ...
              <tbody>
                {loadingAgentes ? (
                  <tr><td colSpan={6} className="text-center py-8 text-gray-900">Carregando agentes...</td></tr>
                  ...
                  agentesComDocs.map((agente, i) => (
                    ...
                    >
                      ...
                      <td className="px-4 py-3 text-gray-900">{agente.situacao ?? '—'}</td>
                      ...
                      <td className="px-4 py-3 text-center">
                        <ChevronRight className="w-4 h-4 text-gray-900 mx-auto" />
                      </td>
                    </tr>
                  ))
                )}
