...
function BuscaAgentesSugestoes({ termo, onSelect }: {
  ...
  if (isLoading) {
    return (
      <div className="absolute top-full left-0 right-0 z-50 bg-white border border-gray-700 rounded-md shadow-lg mt-1 p-2 text-xs text-gray-700">
        Buscando...
      </div>
    );
  }
    ...
    <div className="absolute top-full left-0 right-0 z-50 bg-white border border-gray-700 rounded-md shadow-lg mt-1 max-h-48 overflow-y-auto">
      {data.map((ag: any) => (
        ...
        >
          ...
          {ag.empresa && <span className="text-xs text-gray-700 ml-auto shrink-0">{ag.empresa}</span>}
        </button>
      ))}
    </div>
  );
      ...
      {isLoading ? (
        ...
        <Card>
          <CardContent className="flex flex-col items-center justify-center py-16 gap-3">
            ...
            <p className="text-gray-700 font-medium">Nenhuma operação encontrada para {mesRef}</p>
            <p className="text-gray-700 text-sm">Verifique se há produção importada para o mês de referência.</p>
          </CardContent>
        </Card>
      ) : (
        <Card>
          <CardContent className="p-0">
            ...
            <div className="border-t bg-gray-800 px-4 py-3 flex items-center justify-between">
              <span className="text-sm text-gray-700">{rows.length} operação(ões)</span>
              <div className="flex gap-6">
                <div className="text-right">
                  <p className="text-xs text-gray-700">Total Valor Líquido</p>
                  <p className="font-bold text-blue-700">{fmt(totalLiquido)}</p>
                </div>
                <div className="text-right">
                  <p className="text-xs text-gray-700">Total Comissão</p>
...
function PerspectivadeGanho() {
  ...
  return (
    <div>
      ...
      {/* Nota de edição */}
      <div className="mb-3 text-xs text-gray-700 flex items-center gap-1">
        ...
        <CardContent className="p-0">
          <div className="overflow-x-auto">
            <Table>
              ...
              <TableBody>
                {isLoading ? (
                  <TableRow>
                    <TableCell colSpan={11} className="text-center py-10 text-gray-700">Carregando...</TableCell>
                  </TableRow>
                ) : rows.length === 0 ? (
                  <TableRow>
                    <TableCell colSpan={11} className="text-center py-10 text-gray-700">
                      {periodoInicio && periodoFim
                  ...
                  (rows as any[]).map((row: any, rowIdx: number) => (
                    <TableRow key={row.id} className={rowIdx % 2 === 0 ? 'bg-white hover:bg-blue-900/30' : 'bg-blue-900/20/30 hover:bg-blue-100/40'}>
                      ...
                      <TableCell className="text-sm">
                        ...
                        {row.cpfCliente && <div className="text-xs text-gray-700 font-mono">{row.cpfCliente}</div>}
                      </TableCell>
                      <TableCell>
                        ...
                        {editandoTelefoneId === row.id ? (
                          ...
                          >
                            {row.telefoneManuais ? (
                              ...
                              <span className="text-gray-700 italic">+ telefone</span>
                            )}
                          </button>
                        )}
                      </TableCell>
                        ...
                        {row.perspectivaComissao != null ? (
                          <div>
                            ...
                            {row.percentualUsado != null && (
                              <div className="text-xs text-gray-700 font-normal">{fmtPct(row.percentualUsado)}</div>
                            )}
                          </div>
                        ) : (
          ...
          <div className="border-t bg-gray-800 px-4 py-3 flex items-center justify-between flex-wrap gap-2">
            <div className="flex items-center gap-3">
              <span className="text-sm text-gray-700">{rows.length} contrato(s)</span>
            ...
            <div className="flex gap-6">
              <div className="text-right">
                <p className="text-xs text-gray-700">Total Valor Solicitado</p>
                <p className="font-bold text-blue-700">{fmt(totalValorSolicitado)}</p>
              </div>
              <div className="text-right">
                <p className="text-xs text-gray-700">Total Comissão Perspectiva</p>
    ...
    <div>
      ...
      {isLoading ? (
        ...
        <Card><CardContent className="p-0">
          ...
          <div className="border-t bg-gray-800 px-4 py-3 flex items-center justify-between">
            <span className="text-sm text-gray-700">{rows.length} registro(s)</span>
            <div className="text-right">
              <p className="text-xs text-gray-700">Total Comissão</p>
              <p className="font-bold text-green-700">{fmt(totalComissao)}</p>
            </div>
    ...
    <div>
      ...
      <Card>
        <CardContent className="flex flex-col items-center justify-center py-20 gap-4">
          ...
          <p className="text-gray-700 text-sm">Módulo em desenvolvimento. Em breve disponível.</p>
        </CardContent>
      </Card>
    </div>
  );
  ...
  return (
    <div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
        ...
        <div className="px-4 py-3 bg-amber-50 border-2 border-amber-300 rounded-lg">
          ...
          {isLoading ? (
            ...
            <p className="text-sm text-gray-700">Nenhuma faixa atingida no mês anterior.</p>
          )}
        </div>
          ...
          {isLoading ? (
            ...
            <div className="flex flex-wrap items-center gap-3">
              {nivelVigente ? (
                ...
                <span className="text-sm text-gray-700">Nenhuma faixa atingida ainda</span>
              )}
      ...
      {niveisConfigurados.length > 0 && (
        <div className="mb-4">
          <p className="text-xs font-semibold text-gray-700 uppercase tracking-wide mb-2">Faixas de Nível — Produza mais para ganhar mais!</p>
              ...
              return (
                ...
                >
                  <div className={`text-xs font-bold uppercase mb-0.5 ${
                    isAtualAnt ? 'text-amber-700' : isAtualVig ? 'text-green-700' : isAlcancadoVig ? 'text-blue-600' : 'text-gray-700'
                  }`}>
                  ...
                  <div className={`text-[11px] font-medium ${
                    isAtualAnt ? 'text-amber-800' : isAtualVig ? 'text-green-800' : isAlcancadoVig ? 'text-blue-700' : 'text-gray-700'
                  }`}>
                    {de > 0 ? fmt(de) : '—'}
                    {ate > 0 ? <><br/><span className="text-[10px] text-gray-700">até</span><br/>{fmt(ate)}</> : ''}
                  </div>
                </div>
              );
            })}
      ...
      <Card>
        <CardContent className="p-0">
          {isLoading ? (
            <div className="flex items-center justify-center py-20 text-gray-700">Carregando...</div>
          ) : tabela.length === 0 ? (
            <div className="flex items-center justify-center py-20 text-gray-700">Nenhum registro encontrado para sua empresa.</div>
          ) : (
