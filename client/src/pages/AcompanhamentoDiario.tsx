    ...
    return (
      <div className="min-h-screen bg-gray-950 flex flex-col items-center justify-center gap-4">
        <div className="bg-gray-900 border border-gray-700 rounded-2xl p-10 flex flex-col items-center gap-4 max-w-sm text-center">
          ...
          <p className="text-gray-700 text-sm">Você não tem permissão para acessar o Acompanhamento Diário.<br/>Solicite ao administrador.</p>
  ...
  return (
    <div className="min-h-screen bg-gray-950 text-white">
        ...
        <div className="flex items-center justify-between flex-wrap gap-3">
          ...
          <div className="flex items-center gap-3 flex-wrap">
            ...
            <div className="flex items-center gap-2">
              <Button variant="ghost" size="icon" onClick={prevMes} className="text-gray-700 hover:text-white">
                <ChevronLeft className="w-4 h-4" />
              </Button>
              <div className="text-center min-w-[120px]">
                <div className="font-bold text-lg">{MESES[mes - 1]}</div>
                <div className="text-xs text-gray-700">{ano}</div>
              </div>
              <Button variant="ghost" size="icon" onClick={nextMes} className="text-gray-700 hover:text-white">
                ...
                <button
                  ...
                  className={`px-3 py-1.5 rounded-lg text-sm font-semibold transition-colors ${
                    ano === a
                      ...
                      : 'bg-gray-800 text-gray-700 hover:bg-gray-700 hover:text-white'
                  }`}
                >
                  {a}
                </button>
      ...
      <div className="px-6 py-3 bg-gray-900/50 border-b border-gray-800 flex flex-wrap gap-6 text-sm">
        <div>
          <span className="text-gray-700">Total do mês: </span>
          <span className="font-bold text-green-400">{fmt(totalGeral)}</span>
        </div>
        <div>
          <span className="text-gray-700">Agentes: </span>
          <span className="font-bold">{agentes.length}</span>
        </div>
        <div>
          <span className="text-gray-700">Dias úteis: </span>
        ...
        <div>
          <span className="text-gray-700">Ranking ≥50%: </span>
      ...
      {!isLoading && agentes.length > 0 && (
        <div className="px-4 py-4 space-y-6">
              ...
              <div className="flex flex-wrap gap-2">
                {ranking.map((a, i) => (
                  <div key={a.chaveJ} className="flex items-center gap-2 bg-gray-800 rounded-lg px-3 py-1.5">
                    <span className={`text-xs font-bold ${i === 0 ? "text-yellow-400" : i === 1 ? "text-gray-300" : i === 2 ? "text-amber-600" : "text-gray-700"}`}>
                      #{i+1}
                    </span>
              ...
              <table className="w-full text-xs">
                <thead>
                  ...
                  <tr className="border-b border-gray-700">
                    <th className="sticky left-0 bg-gray-900 z-10 px-3 py-2 text-left text-gray-700 font-medium min-w-[140px]">Agente</th>
                    <th className="px-2 py-2 text-gray-700 font-medium min-w-[60px]">Situação</th>
                    <th className="px-2 py-2 text-gray-700 font-medium min-w-[50px]">Dias C/</th>
                    <th className="px-2 py-2 text-gray-700 font-medium min-w-[50px]">Dias S/</th>
                    <th className="px-2 py-2 text-gray-700 font-medium min-w-[55px]">Aprov.</th>
                    <th className="px-2 py-2 text-gray-700 font-medium min-w-[55px]">Média/DU</th>
                        ...
                        <th key={d} className={`px-1 py-1 text-center min-w-[52px] ${isWkd ? "bg-red-950/40" : zebraUtil}`}>
                          {isSab ? (
                            ...
                            <>
                              ...
                              <div className="text-[9px] text-gray-700">{getDayOfWeek(ano, mes, d)}</div>
                            </>
                          )}
                        </th>
                      );
                ...
                <tbody>
                  {agentes.map((a, idx) => (
                    <tr key={a.chaveJ} className={`border-b border-gray-800/50 hover:bg-gray-800/30 transition-colors ${idx % 2 === 0 ? "" : "bg-gray-900/30"}`}>
                      ...
                      <td className="px-2 py-2 text-center text-gray-700">{a.diasSemProducao}</td>
                      <td className="px-2 py-2 text-center">
          ...
          {/* Legenda */}
          <div className="flex flex-wrap gap-4 text-xs text-gray-700 pb-4">
