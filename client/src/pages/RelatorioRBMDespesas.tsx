  ...
  return (
    <div className="min-h-screen bg-gray-100">
      ...
      <div className="px-3 py-3">
        {isLoading && <div className="text-center py-16 text-gray-800">Carregando...</div>}
        {!isLoading && data.length === 0 && <div className="text-center py-16 text-gray-800">Nenhum dado encontrado para {ano}.</div>}
        ...
        {!isLoading && data.length > 0 && (
          <>
                  ...
                  <div key={`${r.chaveJ}-${r.empresa}`} className="bg-gray-900 rounded-lg border border-gray-700 overflow-hidden">
                    ...
                    <div className="flex items-start justify-between px-3 py-2 border-b border-gray-100">
                      <div className="flex items-center gap-2 min-w-0">
                        <span className={`shrink-0 px-2 py-0.5 rounded text-xs font-bold ${r.empresa==='BMF'?'bg-green-100 text-green-800':r.empresa==='FLEX'?'bg-blue-100 text-blue-800':'bg-gray-100 text-gray-800'}`}>
                          {r.empresa}
                        </span>
                        <div className="min-w-0">
                          <div className="font-semibold text-white text-sm truncate">{r.nomeAgente || r.chaveJ}</div>
                          <div className="text-xs text-gray-800">{r.chaveJ} · {r.cidade || "-"}{r.nAgentesNaCidade > 1 && <span className="text-orange-500"> ÷{r.nAgentesNaCidade}</span>} · {r.meses} {r.meses === 1 ? "mês" : "meses"}</div>
                        </div>
                      </div>
                      {/* Saldo destaque */}
                    ...
                    <div className="grid grid-cols-3 divide-x divide-gray-100 text-xs">
                      ...
                      <div className="px-3 py-2">
                        <div className="text-[10px] text-gray-800 font-medium mb-1 uppercase tracking-wide">RBM</div>
                        <div className="font-bold text-blue-800 text-sm">{fmtBRL(r.rbmTotal)}</div>
                        {r.rbmConsig > 0 && <div className="text-gray-800 mt-0.5">Consig: <span className="text-blue-600">{fmtBRL(r.rbmConsig)}</span></div>}
                        {r.rbmCC > 0 && <div className="text-gray-800">C/C: <span className="text-blue-600">{fmtBRL(r.rbmCC)}</span></div>}
                        {r.rbmConsorcio > 0 && <div className="text-gray-800">Consórc: <span className="text-blue-600">{fmtBRL(r.rbmConsorcio)}</span></div>}
                        {r.rbmOurocap > 0 && <div className="text-gray-800">Ouro: <span className="text-blue-600">{fmtBRL(r.rbmOurocap)}</span></div>}
                        {r.rbmSeguros > 0 && <div className="text-gray-800">Seg: <span className="text-blue-600">{fmtBRL(r.rbmSeguros)}</span></div>}
                      </div>

                      {/* Comissões */}
                      <div className="px-3 py-2">
                        <div className="text-[10px] text-gray-800 font-medium mb-1 uppercase tracking-wide">Comissões</div>
                        <div className="font-bold text-green-700 text-sm">{fmtBRL(r.comissaoTotal)}</div>
                      </div>

                      {/* Despesas + % */}
                      <div className="px-3 py-2">
                        <div className="text-[10px] text-gray-800 font-medium mb-1 uppercase tracking-wide">Despesas</div>
                        <div className={`font-bold text-sm ${r.totalDesp > 0 ? "text-orange-700" : "text-gray-800"}`}>{fmtBRL(r.totalDesp)}</div>
                        {r.totalDespFixas > 0 && <div className="text-gray-800 mt-0.5">Fixas: <span className="text-orange-600">{fmtBRL(r.totalDespFixas)}</span></div>}
                        {r.totalDespAvulsas > 0 && <div className="text-gray-800">Avulsas: <span className="text-orange-600">{fmtBRL(r.totalDespAvulsas)}</span></div>}
                        <div className={`mt-1 text-xs font-semibold ${pctAlto ? "text-red-600" : pctMedio ? "text-orange-500" : "text-gray-800"}`}>
            ...
            <div className="mt-4 bg-gray-800 text-white rounded-lg p-3">
              <div className="text-xs font-bold mb-2 uppercase tracking-wide text-gray-800">Totais Gerais — {data.length} agentes</div>
              <div className="grid grid-cols-2 md:grid-cols-4 gap-3 text-sm">
                <div>
                  <div className="text-[10px] text-gray-800 uppercase">RBM Total</div>
                  <div className="font-bold text-blue-300">{fmtBRL(totais.rbmTotal)}</div>
                </div>
                <div>
                  <div className="text-[10px] text-gray-800 uppercase">Comissões</div>
                  <div className="font-bold text-green-300">{fmtBRL(totais.comissaoTotal)}</div>
                </div>
                <div>
                  <div className="text-[10px] text-gray-800 uppercase">Despesas</div>
                  <div className="font-bold text-orange-300">{fmtBRL(totais.totalDesp)}</div>
                </div>
                <div>
                  <div className="text-[10px] text-gray-800 uppercase">Saldo</div>
