    ...
    <div className="min-h-screen bg-gray-950 text-white">
      ...
      {mostrarTotalizador && (
        <div className="px-6 py-4 bg-gradient-to-r from-green-50 to-emerald-50 border-b">
          <div className="mb-4">
            <h2 className="text-sm font-semibold text-gray-900 mb-3">📊 Totalizador por Empresa e Mês/Ano</h2>
          ...
          {totalizador && (mesAnoTotalizador || empresaTotalizador) && (
            <div className="grid grid-cols-3 gap-4 mt-4 pt-4 border-t border-green-200">
              ...
              <div className="bg-gray-900 rounded-lg p-3 shadow-sm">
                ...
                <p className="text-lg font-bold text-gray-900">{totalizador.registros}</p>
              </div>
            </div>
          )}
        </div>
      ...
      <div className="px-6 py-4 bg-gray-900 border-b border-gray-700">
        <div className="flex flex-wrap gap-3 items-center">
          ...
          <span className="text-sm text-gray-900 ml-auto">{registrosFiltrados.length} registro(s)</span>
        </div>
      </div>
        ...
        {isLoading ? (
          ...
          <table className="w-full text-xs border-collapse min-w-[600px]">
            ...
            <tbody>
              {registrosFiltrados.length === 0 && (
                <tr>
                  <td colSpan={6} className="text-center py-10 text-gray-900">
                ...
                >
                  ...
                  <td className="px-2 py-1.5 border-b border-gray-100">
                    <div className="flex items-center gap-1.5 mb-0.5">
                      ...
                      <span className="text-[9px] text-gray-900">{mesNumParaStr(r.mes)}</span>
                    </div>
                    ...
                    {r.supervisor && <div className="text-[10px] text-gray-900">Sup: {r.supervisor}</div>}
                  </td>
                  {/* Coluna Operação + Produto */}
                  <td className="px-2 py-1.5 border-b border-gray-100">
                    <div className="text-[11px] text-gray-900 font-medium">{strVal(r.convenio)}</div>
                    <div className={`font-mono text-[10px] ${r.isDuplicate ? 'text-red-700 font-bold' : 'text-gray-900'}`}>
                      {strVal(r.nrOperacao)}{r.isDuplicate && <span className="ml-1">⚠️</span>}
                    </div>
                    {r.dtContratacao && <div className="text-[10px] text-gray-900">{strVal(r.dtContratacao)}</div>}
                    <div className="text-[10px] text-gray-300 font-medium mt-0.5">{strVal(r.produto)}</div>
                    {r.descricaoProduto && <div className="text-[10px] text-gray-900 max-w-[150px] truncate" title={r.descricaoProduto}>{r.descricaoProduto}</div>}
                  ...
                  <td className="px-2 py-1.5 border-b border-gray-100 text-right whitespace-nowrap">
                    ...
                    <div className="text-[10px] text-gray-900">Bruto: {moeda(r.valorBruto)}</div>
                    <div className="text-[10px] text-gray-900">RBM: {moeda(r.rbm)}</div>
                    {r.parcela != null && <div className="text-[10px] text-gray-900">Parc: {r.parcela}</div>}
                  </td>
                  {/* Coluna Comissão */}
                  <td className="px-2 py-1.5 border-b border-gray-100 text-right whitespace-nowrap">
                    ...
                    {r.tabela && r.tabela !== 'NULL' && r.tabela !== '0' && (
                      <div className="text-[10px] text-gray-900">{r.tabela.replace(/^Ativo(\d+)$/, 'Ativo $1').replace(/^Tabela(\d+)$/, 'Tabela $1')}</div>
                    )}
                  </td>
                  <td className="px-2 py-1.5 border-b border-gray-100">
