...
export default function TabelaComissao() {
  ...
  function getConvenioColor(convenio: string | null) {
    if (!convenio) return { row: 'bg-white hover:bg-gray-800', badge: 'bg-gray-100 text-gray-700 border border-gray-300' };
  ...
  return (
    <div className="min-h-screen bg-gray-950 text-white">
      ...
      <div className="px-6 py-1 text-xs text-gray-700">{filteredRows.length} registros</div>
      ...
      <div className="px-6 py-4 bg-gray-900 border-b border-gray-700">
        <div className="flex flex-wrap gap-3 items-center">
          <div className="flex items-center gap-2 flex-1 min-w-[200px]">
            ...
            />
            {busca && (
              <button onClick={() => setBusca('')} className="text-gray-700 hover:text-gray-300">
                <X className="w-4 h-4" />
              </button>
            )}
          </div>
              ...
              {showColSelector && (
                <div className="absolute right-0 top-full mt-1 z-50 bg-white border border-gray-700 rounded-xl shadow-xl p-3 w-52">
                  <div className="flex items-center justify-between mb-2">
                    ...
                    <button onClick={() => setShowColSelector(false)} className="text-gray-700 hover:text-gray-300">
        ...
        <Card className="overflow-hidden">
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              ...
              <tbody>
                {isLoading ? (
                  <tr><td colSpan={isAdminOuCeo ? 13 : ativoAgente ? 4 : 2} className="text-center py-8 text-gray-700">Carregando...</td></tr>
                ) : filteredRows.length === 0 ? (
                  <tr><td colSpan={isAdminOuCeo ? 13 : ativoAgente ? 4 : 2} className="text-center py-8 text-gray-700">Nenhum registro encontrado</td></tr>
                ) : (
                  filteredRows.map((row, rowIdx) => {
                    ...
                    return (
                      <tr key={row.id} className={`${convColor.row} transition-colors`}>
                        {/* Número de linha */}
                        <td className="px-2 py-2 text-center text-xs text-gray-700 font-mono w-8 select-none">{rowIdx + 1}</td>
                        ...
                        <td className="px-3 py-2" style={{width:'220px',maxWidth:'220px'}}>
                          <div className="flex items-center gap-1 text-xs mb-0.5">
                            ...
                            {(row as any).codigo && <span className="text-gray-700">· {(row as any).codigo}</span>}
                          </div>
                        ...
                        <td className="px-3 py-2 whitespace-nowrap">
                          {/* Juros */}
                          <div className="flex items-center gap-1 mb-0.5">
                            <span className="text-gray-700 text-xs">Juros:</span>
                            {isAdminOuCeo ? (
                          ...
                          {/* Valor Mínimo */}
                          <div className="flex items-center gap-1 mb-0.5">
                            <span className="text-gray-700 text-xs">Mín:</span>
                            {isAdminOuCeo ? (
                          ...
                          <div className="flex items-center gap-1">
                            <span className="text-gray-700 text-xs">Prazo:</span>
                            {isAdminOuCeo ? (
      ...
      <Dialog open={modalOpen} onOpenChange={setModalOpen}>
        <DialogContent className="max-w-3xl max-h-[90vh] overflow-y-auto">
          ...
          <div className="grid grid-cols-2 gap-4 py-2">
            ...
            <div>
              <label className="text-sm font-medium text-gray-700 mb-1 block">Código <span className="text-gray-700 text-xs">(até 5 códigos separados por /)</span></label>
