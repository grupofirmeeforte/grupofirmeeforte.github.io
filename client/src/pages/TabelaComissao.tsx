...
export default function TabelaComissao() {
  ...
  function getConvenioColor(convenio: string | null) {
    if (!convenio) return { row: 'bg-white hover:bg-gray-800', badge: 'bg-gray-100 text-gray-800 border border-gray-300' };
  ...
  return (
    <div className="min-h-screen bg-gray-950 text-white">
      ...
      <div className="px-6 py-1 text-xs text-gray-800">{filteredRows.length} registros</div>
      ...
      <div className="px-6 py-4 bg-gray-900 border-b border-gray-700">
        <div className="flex flex-wrap gap-3 items-center">
          <div className="flex items-center gap-2 flex-1 min-w-[200px]">
            ...
            />
            {busca && (
              <button onClick={() => setBusca('')} className="text-gray-800 hover:text-gray-800">
                <X className="w-4 h-4" />
              </button>
            )}
          </div>
              ...
              {showColSelector && (
                <div className="absolute right-0 top-full mt-1 z-50 bg-white border border-gray-700 rounded-xl shadow-xl p-3 w-52">
                  <div className="flex items-center justify-between mb-2">
                    <span className="text-xs font-semibold text-gray-800 uppercase tracking-wide">Ativos visíveis</span>
                    <button onClick={() => setShowColSelector(false)} className="text-gray-800 hover:text-gray-800">
        ...
        <Card className="overflow-hidden">
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              ...
              <tbody>
                {isLoading ? (
                  <tr><td colSpan={isAdminOuCeo ? 13 : ativoAgente ? 4 : 2} className="text-center py-8 text-gray-800">Carregando...</td></tr>
                ) : filteredRows.length === 0 ? (
                  <tr><td colSpan={isAdminOuCeo ? 13 : ativoAgente ? 4 : 2} className="text-center py-8 text-gray-800">Nenhum registro encontrado</td></tr>
                ) : (
                  filteredRows.map((row, rowIdx) => {
                    ...
                    return (
                      <tr key={row.id} className={`${convColor.row} transition-colors`}>
                        {/* Número de linha */}
                        <td className="px-2 py-2 text-center text-xs text-gray-800 font-mono w-8 select-none">{rowIdx + 1}</td>
                        ...
                        <td className="px-3 py-2" style={{width:'220px',maxWidth:'220px'}}>
                          <div className="flex items-center gap-1 text-xs mb-0.5">
                            ...
                            {(row as any).codigo && <span className="text-gray-800">· {(row as any).codigo}</span>}
                          </div>
                        ...
                        <td className="px-3 py-2 whitespace-nowrap">
                          {/* Juros */}
                          <div className="flex items-center gap-1 mb-0.5">
                            <span className="text-gray-800 text-xs">Juros:</span>
                            {isAdminOuCeo ? (
                              <span className="inline-flex items-center gap-1 text-sm font-bold text-emerald-600">
                                ...
                                <span className="text-gray-800 font-normal text-xs">→</span>
                          ...
                          {/* Valor Mínimo */}
                          <div className="flex items-center gap-1 mb-0.5">
                            <span className="text-gray-800 text-xs">Mín:</span>
                            {isAdminOuCeo ? (
                              ...
                              <span className="text-xs text-gray-800">{row.valorMinimo || '-'}</span>
                            )}
                          </div>
                          {/* Prazo */}
                          <div className="flex items-center gap-1">
                            <span className="text-gray-800 text-xs">Prazo:</span>
                            {isAdminOuCeo ? (
            ...
            <div className="col-span-2">
              ...
              <div className="grid grid-cols-5 gap-2">
                {['ativo01', 'ativo02', 'ativo03', 'ativo04', 'ativo05', 'ativo06', 'ativo07', 'ativo08', 'ativo09', 'ativo10', 'ativo11', 'ativo12', 'ativo13', 'ativo14', 'ativo15', 'ativo16', 'ativo17', 'ativo18', 'ativo19', 'ativo20'].map((key) => (
                  <div key={key}>
                    <label className="text-xs font-medium text-gray-800 mb-1 block">{key.replace('ativo', 'Ativo ')}</label>
