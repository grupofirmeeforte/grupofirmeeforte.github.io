  ...
  return (
    <div className="min-h-screen bg-gray-950 text-white">
      ...
      <div className="bg-gray-900 border-b border-gray-700 px-6 py-3 flex flex-wrap gap-3 items-end">
        <div>
          <Label className="text-xs text-gray-700">Mês/Ano</Label>
        ...
        <div>
          <Label className="text-xs text-gray-700">Empresa</Label>
        ...
        <div>
          <Label className="text-xs text-gray-700">Tipo Pagto</Label>
        ...
        <div>
          <Label className="text-xs text-gray-700">Pago</Label>
        ...
        <div>
          <Label className="text-xs text-gray-700">Chave J</Label>
        ...
        {(filtroMesAno || filtroEmpresa || filtroTipo || filtroChaveJ || filtroNome || filtroPago !== "todos") && (
          ...
          }} className="text-gray-700 hover:text-white h-8 text-xs">✕ Limpar</Button>
        )}
      </div>
      ...
      <div className="bg-gray-800 border-b border-gray-700 px-6 py-3">
        <div className="flex items-center gap-4 flex-wrap">
          ...
          {resumo && (
            <div className="flex gap-3 flex-wrap text-[11px]">
              ...
              <div className="flex items-center gap-1">
                <span className="text-gray-700">Aluguéis:</span>
        ...
        <div className="text-[10px] text-gray-700 mt-1">Vermelho = a pagar | Verde = pago | Filtro por data de pagamento &nbsp;—&nbsp; {total} registro{total !== 1 ? "s" : ""}</div>
      </div>
      ...
      <div className="overflow-x-auto">
        <table className="w-full text-xs border-collapse">
          ...
          <tbody>
            {rows.length === 0 ? (
              <tr>
                <td colSpan={5} className="text-center py-12 text-gray-700">
              ...
              <tr key={`${row._fonte}-${row.id}`}
                ...
                <td className="px-2 py-1.5 whitespace-nowrap">
                  <div className="flex items-center gap-1.5 mb-0.5 flex-wrap">
                    ...
                    {row.mesAno && <span className="text-[9px] text-gray-700">{row.mesAno}</span>}
                  </div>
                  ...
                  <div className="text-[10px] text-gray-700">{row.empresa || ""}{row.cidadeUF ? ` · ${row.cidadeUF}` : ""}</div>
                </td>
                ...
                <td className="px-2 py-1.5 whitespace-nowrap text-[10px]">
                  <div className="text-gray-300">
                    ...
                    {row.tipoConta && <span className="ml-1 text-gray-700">({row.tipoConta})</span>}
                  </div>
                  {row.cpfCnpj && <div className="text-gray-700 font-mono">{row.cpfCnpj}</div>}
                  {row.pix && (
                  ...
                  <div className="text-[10px] text-gray-700">
                    {editandoDtPagto === row.id ? (
                      ...
                      <span onClick={() => iniciarEdicaoDtPagto(row, i)}
                        ...
                        Pagto: {row.dataPagto || <span className="text-gray-700 italic">DD/MM/AAAA</span>}
                      </span>
                    )}
                  </div>
                  {row.dataVencer && (
                    <div className="text-[10px] mt-0.5">
                      {!row.pago && !row.dataPagto && isAtrasado(row.dataVencer)
                        ...
                        : !row.pago && !row.dataPagto && isHoje(row.dataVencer)
                          ...
                          : <span className="text-gray-700">Vence: {row.dataVencer}</span>
                      }
                    </div>
                  )}
                </td>
      ...
      <Dialog open={showModal} onOpenChange={setShowModal}>
        <DialogContent className="bg-gray-900 border-gray-700 text-white max-w-3xl max-h-[90vh] overflow-y-auto">
          ...
          <div className="grid grid-cols-2 gap-3 py-2">
            ...
            <div>
              <Label className="text-xs text-gray-700">Cadastro</Label>
              <Input value={form.cadastro} onChange={e => setForm(f => ({ ...f, cadastro: e.target.value }))}
                className="bg-gray-800 border-gray-600 text-white h-8 text-sm" />
            </div>
            <div>
              <Label className="text-xs text-gray-700">Cidade/UF</Label>
            ...
            <div className="col-span-2">
              <Label className="text-xs text-gray-700">Nome Favorecido</Label>
            ...
            <div>
              <Label className="text-xs text-gray-700">Tipo Conta</Label>
            ...
            <div>
              <Label className="text-xs text-gray-700">CPF/CNPJ</Label>
            ...
            <div>
              <Label className="text-xs text-gray-700">Tipo Chave</Label>
            ...
            <div>
              <Label className="text-xs text-gray-700">{((form as any).tipoChave ?? 'pix') === 'boleto' ? 'Código de Barras' : 'Pix'}</Label>
            ...
            <div>
              <Label className="text-xs text-gray-700">Valor (R$)</Label>
              <Input value={form.valor} onChange={e => setForm(f => ({ ...f, valor: e.target.value }))}
                placeholder="0,00" className="bg-gray-800 border-gray-600 text-white h-8 text-sm" />
            </div>
            <div>
              <Label className="text-xs text-gray-700">Data Vencer</Label>
            ...
            {form.pago && (
              <div>
                <Label className="text-xs text-gray-700">Data Pagto</Label>
                <Input value={form.dataPagto} onChange={e => setForm(f => ({ ...f, dataPagto: maskData(e.target.value) }))}
                  placeholder="DD/MM/AAAA" maxLength={10} className="bg-gray-800 border-gray-600 text-white h-8 text-sm" />
              </div>
            )}
            <div className="col-span-2">
              <Label className="text-xs text-gray-700">Observação</Label>
      ...
      <Dialog open={editandoDesp !== null} onOpenChange={() => setEditandoDesp(null)}>
        <DialogContent className="bg-gray-900 border-gray-700 text-white max-w-2xl max-h-[90vh] overflow-y-auto">
          ...
          <div className="grid grid-cols-2 gap-3 py-2">
            ...
            <div>
              <Label className="text-xs text-gray-700">Pix</Label>
            ...
            <div>
              <Label className="text-xs text-gray-700">Cidade Resp.</Label>
