          ...
          <div key={m.modulo} className="border rounded-lg overflow-hidden">
            ...
            >
              <div className="flex items-center gap-2">
                {aberto ? <ChevronUp className="w-3 h-3 text-gray-800" /> : <ChevronDown className="w-3 h-3 text-gray-800" />}
    ...
    <div className="space-y-4">
      ...
      {aba === 'individuais' && (
        <div className="bg-white rounded-xl border border-gray-200 shadow-sm">
            ...
            <Table>
              ...
              <TableBody>
                {isLoading ? (
                  ...
                  <TableRow><TableCell colSpan={4} className="text-center py-8 text-slate-800">Nenhum agente encontrado.</TableCell></TableRow>
                ) : listaFiltrada.map(a => (
                  <AgentePermissaoRow
                    key={a.id}
                    agente={a}
          ...
          <p className="text-xs text-slate-800 p-3">{listaFiltrada.length} agente(s)</p>
        </div>
      )}
    </div>
  );
