  ...
  return (
    <div className="space-y-6 p-6">
      ...
      <div className="flex gap-1 border-b border-gray-700">
        <button
          onClick={() => setAba('logs')}
          className={`flex items-center gap-2 px-5 py-2.5 text-sm font-medium rounded-t-lg transition-colors ${aba === 'logs' ? 'bg-white border border-b-white border-gray-700 text-blue-700' : 'text-gray-700 hover:text-gray-700'}`}
        >
          <ClipboardList className="w-4 h-4" /> Logs de Acesso
        </button>
        <button
          onClick={() => setAba('feriados')}
          className={`flex items-center gap-2 px-5 py-2.5 text-sm font-medium rounded-t-lg transition-colors ${aba === 'feriados' ? 'bg-white border border-b-white border-gray-700 text-blue-700' : 'text-gray-700 hover:text-gray-700'}`}
        >
          <CalendarDays className="w-4 h-4" /> Feriados
        </button>
        <button
          onClick={() => setAba('credito-despesas')}
          className={`flex items-center gap-2 px-5 py-2.5 text-sm font-medium rounded-t-lg transition-colors ${aba === 'credito-despesas' ? 'bg-white border border-b-white border-gray-700 text-blue-700' : 'text-gray-700 hover:text-gray-700'}`}
        >
          <BarChart2 className="w-4 h-4" /> Crédito x Despesas
        </button>
        <button
          onClick={() => setAba('permissoes')}
          className={`flex items-center gap-2 px-5 py-2.5 text-sm font-medium rounded-t-lg transition-colors ${aba === 'permissoes' ? 'bg-white border border-b-white border-gray-700 text-blue-700' : 'text-gray-700 hover:text-gray-700'}`}
        >
        ...
        <div className="space-y-4">
          ...
          {sessoesAtivas && sessoesAtivas.length > 0 && (
            <div className="bg-white border border-emerald-200 rounded-lg p-4">
              <div className="flex items-center justify-between mb-3">
                ...
                <Button variant="ghost" size="sm" onClick={() => refetchSessoes()} className="h-7 gap-1 text-xs text-gray-700">
              ...
              <div className="flex flex-wrap gap-2">
                {sessoesAtivas.map((s: any) => (
                  <div key={s.id} className="flex items-center gap-2 bg-emerald-50 border border-emerald-200 rounded-lg px-3 py-1.5">
                    ...
                    <span className="text-[10px] text-gray-700">{tempoOnline(s.ultimoAcesso)}</span>
                    {isCeo && (
                      <button
          ...
          <div className="bg-gray-900 rounded-lg border border-gray-700 overflow-x-auto shadow-sm">
            <table className="w-full text-xs">
              ...
              <tbody>
                {loadingLogs ? (
                  <tr><td colSpan={5} className="text-center py-6 text-gray-700">Carregando...</td></tr>
                ) : logs && logs.length > 0 ? (
                  logs.map((log: any, idx: number) => (
                    <tr key={log.id} className={idx % 2 === 0 ? 'bg-white hover:bg-blue-900/30' : 'bg-blue-900/20/30 hover:bg-blue-100/40'}>
                      ...
                      <td className="px-2 py-1">
                        ...
                        {log.ipAddress && <div className="text-[9px] text-gray-700 leading-tight">{log.ipAddress}</div>}
                      </td>
                      {/* Coluna Módulo/Ação */}
                      <td className="px-2 py-1">
                        {log.modulo && (
                      ...
                      <td className="px-2 py-1 whitespace-nowrap">
                        ...
                        <div className="text-[9px] text-gray-700 leading-tight">{new Date(log.horarioEntrada).toLocaleTimeString('pt-BR', { hour: '2-digit', minute: '2-digit', second: '2-digit' })}</div>
                      </td>
                      {/* Coluna Duração */}
                      <td className="px-2 py-1 whitespace-nowrap">
                        {fmtDuracao(log.horarioEntrada, log.horarioSaida)
                          ...
                          : <span className="text-[9px] text-gray-700">andamento</span>
                        }
                      </td>
                      {/* Coluna Descrição */}
                      <td className="px-2 py-1">
                        <div className="text-[10px] text-gray-300 truncate max-w-[320px]" title={log.descricao || ''}>{log.descricao || '-'}</div>
                        <div className="text-[9px] text-gray-700 font-mono leading-tight">{log.numeroEntrada}</div>
                      </td>
                    </tr>
                  ))
                ) : (
                  <tr><td colSpan={5} className="text-center py-6 text-gray-700">Nenhum registro encontrado</td></tr>
                )}
              </tbody>
            </table>
          </div>
          ...
          {/* Paginação rodapé */}
          <div className="flex justify-between items-center">
            <p className="text-sm text-gray-700">Página {page + 1} de {Math.max(1, totalPages)}</p>
            <div className="flex gap-1">
              <Button onClick={() => setPage(0)} disabled={page === 0} variant="outline" size="sm" className="h-8 w-8 p-0"><ChevronFirst className="w-4 h-4" /></Button>
              <Button onClick={() => setPage(p => Math.max(0, p - 1))} disabled={page === 0} variant="outline" size="sm" className="h-8 w-8 p-0"><ChevronLeft className="w-4 h-4" /></Button>
              <Button onClick={() => setPage(p => Math.min(totalPages - 1, p + 1))} disabled={page >= totalPages - 1} variant="outline" size="sm" className="h-8 w-8 p-0"><ChevronRight className="w-4 h-4" /></Button>
      ...
      {aba === 'feriados' && (
        <div className="space-y-4">
          <div className="flex justify-between items-center">
            <p className="text-sm text-gray-700">{feriadosList ? `${feriadosList.length} feriados em ${filtroAno}` : 'Carregando...'}</p>
          ...
          <div className="flex justify-between items-center">
            <p className="text-sm text-gray-700">Ano: {filtroAno}</p>
            <div className="flex gap-1">
            ...
            <CardContent className="p-0">
              <div className="overflow-x-auto">
                <Table>
                  ...
                  <TableBody>
                    {loadingFeriados ? (
                      <TableRow><TableCell colSpan={6} className="text-center py-8 text-gray-700">Carregando...</TableCell></TableRow>
                      ...
                      <TableRow><TableCell colSpan={6} className="text-center py-8 text-gray-700">Nenhum feriado encontrado para {filtroAno}</TableCell></TableRow>
                    )}
                  </TableBody>
                </Table>
              </div>
      ...
      {aba === 'credito-despesas' && (
        <div className="space-y-4">
          ...
          <div className="flex flex-wrap gap-3 items-end">
            ...
            <p className="text-sm text-gray-700 ml-auto">{cdDados ? `${cdDados.length} registro(s)` : ''}</p>
          </div>
          ...
          <div className="overflow-x-auto rounded-lg border border-gray-700 shadow-sm">
            <table className="min-w-max text-xs border-collapse">
              ...
              <tbody>
                {cdLoading ? (
                  <tr><td colSpan={28} className="text-center py-8 text-gray-700">Carregando...</td></tr>
                ) : !cdDados || cdDados.length === 0 ? (
                  <tr><td colSpan={28} className="text-center py-8 text-gray-700">Nenhum registro encontrado</td></tr>
                ) : (
                  cdDados.map((row, i) => (
  ...
  return (
    <button
      ...
      className={`flex items-center gap-2 px-5 py-2.5 text-sm font-medium rounded-t-lg transition-colors ${
        aba === 'despesas-internas' ? 'bg-white border border-b-white border-gray-700 text-red-700' : 'text-gray-700 hover:text-gray-700'
      }`}
    >
      ...
      <div className="flex flex-col items-center justify-center py-20 gap-6">
        <div className="bg-white border border-gray-700 rounded-2xl shadow-lg p-8 w-full max-w-sm flex flex-col items-center gap-5">
          ...
          <div className="text-center">
            ...
            <p className="text-gray-700 text-sm mt-1">Informe a senha CEO para acessar as Despesas Internas</p>
          </div>
          <div className="w-full space-y-2">
            ...
            <div className="relative">
              ...
              <button
                ...
                className="absolute right-3 top-1/2 -translate-y-1/2 text-gray-700 hover:text-gray-300"
              >
      ...
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-gray-700">Acesso restrito — Diretoria</p>
          </div>
        </div>
        <div className="flex items-center gap-3">
          <div className="flex flex-col gap-1">
          ...
          <CardContent className="p-0">
            <div className="overflow-x-auto">
              <Table>
                ...
                <TableBody>
                  {(despesas ?? []).length === 0 ? (
                    <TableRow><TableCell colSpan={8} className="text-center py-10 text-gray-700">Nenhuma despesa lançada</TableCell></TableRow>
                  ) : (
                    (despesas ?? []).map((d: DespesaInterna) => (
                      <TableRow key={d.id} className="hover:bg-red-50">
                        ...
                        <TableCell className="text-sm text-gray-700 max-w-[120px] truncate">{d.observacao ?? '—'}</TableCell>
                        <TableCell className="text-right">
                          <div className="flex gap-1 justify-end">
            ...
            <div className="border-t bg-red-50 px-4 py-3 flex items-center justify-between">
              <span className="text-sm text-gray-700">{(despesas ?? []).length} registro(s)</span>
              <div className="text-right">
                <p className="text-xs text-gray-700">Total do Período</p>
...
function BloqueiosAbaBtn({ aba, setAba }: { aba: string; setAba: (v: any) => void }) {
  ...
  return (
    <button
      ...
      className={`flex items-center gap-2 px-5 py-2.5 text-sm font-medium rounded-t-lg transition-colors ${
        aba === 'bloqueios' ? 'bg-white border border-b-white border-gray-700 text-red-700' : 'text-gray-700 hover:text-gray-700'
      }`}
    >
  ...
  return (
    <div className="space-y-4">
      <div className="bg-gray-900 rounded-xl border border-red-200 shadow-sm p-5">
        ...
        {isLoading ? (
          <div className="text-center py-8 text-gray-700">Carregando...</div>
...
function BackupAbaBtn({ aba, setAba }: { aba: string; setAba: (v: any) => void }) {
  ...
  return (
    <button
      ...
      className={`flex items-center gap-2 px-5 py-2.5 text-sm font-medium rounded-t-lg transition-colors ${
        aba === 'backup' ? 'bg-white border border-b-white border-gray-700 text-emerald-700' : 'text-gray-700 hover:text-gray-700'
      }`}
    >
      ...
      <div className="bg-white border border-gray-700 rounded-xl p-6 shadow-sm">
        <div className="flex items-center gap-3 mb-4">
          ...
          <div>
            ...
            <p className="text-sm text-gray-700">Exportação completa de todas as tabelas do banco de dados</p>
          </div>
        </div>
        ...
        {ultimoBackup && (
          <div className="bg-gray-800 border border-gray-700 rounded-lg p-4 flex items-center gap-3">
            ...
            <div className="text-sm">
              ...
              <span className="text-gray-700 ml-2">• {ultimoBackup.tabelas} tabelas • {ultimoBackup.tamanhoKB} KB</span>
            </div>
          </div>
        )}
...
function ArquivoMortoAbaBtn({ aba, setAba }: { aba: string; setAba: (v: any) => void }) {
  ...
  return (
    <button
      ...
      className={`flex items-center gap-2 px-5 py-2.5 text-sm font-medium rounded-t-lg transition-colors ${
        aba === 'arquivo-morto' ? 'bg-white border border-b-white border-gray-700 text-amber-700' : 'text-gray-700 hover:text-gray-700'
      }`}
    >
  ...
  return (
    <div className="space-y-4 mt-4">
      ...
      <div className="bg-white border border-gray-700 rounded-xl p-4 shadow-sm flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-gray-700">Armazenamento de arquivos originais (Excel, PDF) por módulo e mês/ano</p>
          </div>
        </div>
        <Button
      ...
      <div className="bg-white border border-gray-700 rounded-xl p-3 flex flex-wrap gap-2">
        <div className="relative">
          <Search className="absolute left-2 top-2.5 w-4 h-4 text-gray-700" />
          <input
        ...
        <table className="w-full text-sm border-collapse">
          ...
          <tbody>
            {isLoading ? (
              ...
              <tr><td colSpan={7} className="text-center py-10 text-gray-700">
              ...
              <tr key={row.id} className={i % 2 === 0 ? "bg-white" : "bg-amber-50/30"}>
                <td className="px-3 py-2">
                  ...
                  {row.descricao && <div className="text-[10px] text-gray-700 mt-0.5">{row.descricao}</div>}
                </td>
                <td className="px-3 py-2">
                ...
                <td className="px-3 py-2 text-xs text-gray-700">{fmtTamanho(row.tamanho)}</td>
                <td className="px-3 py-2 text-xs text-gray-700">{row.uploadadoPor ?? "-"}</td>
      ...
      {totalPages > 1 && (
        <div className="flex items-center justify-between px-2 py-2 text-sm">
          <span className="text-gray-700">Página {page + 1} de {totalPages} — {total} arquivos</span>
          <div className="flex gap-2">
              ...
              >
                {uploadFile ? (
                  <div className="flex items-center justify-center gap-2 text-sm text-gray-700">
                    ...
                    <span className="text-gray-700">({fmtTamanho(uploadFile.size)})</span>
                  </div>
                ) : (
