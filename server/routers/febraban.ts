  ...
  resumo: protectedProcedure
    ...
    .query(async ({ input }) => {
      ...
      for (const emp of empresas) {
        ...
        const anoWhere = sql`empresa = ${emp} AND FLOOR(mesano / 100) = ${anoFull}`;
        ...
        const [contratado, pendente, diaAtual, diaAnterior, ano,
          ...
          // Líquido do ano
          db.select({ total: sql<string>`COALESCE(SUM(CAST(troco AS DECIMAL(15,2))), 0)` })
            .from(febraban).where(sql`empresa = ${emp} AND FLOOR(mesano / 100) = ${anoFull} AND situacao = 'Contratada'`),
          ...
          db.select({ total: sql<string>`COALESCE(SUM(CAST(financiado AS DECIMAL(15,2))), 0)` })
            .from(febraban).where(sql`empresa = ${emp} AND FLOOR(mesano / 100) = ${anoFull} AND (
              pago = 2
              OR EXISTS (
                SELECT 1 FROM consignados c
          ...
          db.select({ total: sql<string>`COALESCE(SUM(CAST(troco AS DECIMAL(15,2))), 0)` })
            .from(febraban).where(sql`empresa = ${emp} AND FLOOR(mesano / 100) = ${anoFull} AND LOWER(situacao) LIKE '%cancel%'`),
